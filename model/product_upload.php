<?php
    ...
    $pro_desc=nl2br($_POST['product_desc']); // nl2br : 엔터로 개행시 br태그 자동 입력
    ...
    // 확장자 필터링
    if(!in_array($exp, $allowed_exp)){ // in_array : 두번재 파라미터 배열에 첫번재 파라미터 문자열이 있으면 true
        echo json_encode(array("msg" => "허용하지 않는 이미지 파일 형식입니다."));
        exit();
    }
